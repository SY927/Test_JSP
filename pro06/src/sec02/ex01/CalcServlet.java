...
public class CalcServlet extends HttpServlet {
   private static float USD_RATE = 1103.47F;
   ...
   protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      if (command != null && command.equals("calculate")) { // 최초 요청시 command가 null이면 계산기 화면 출력, command 값이 calculate면
         String result = calculate(Float.parseFloat(won), operator);// 계산 결과를 출력
   ...
   private static String calculate(float won, String operator) { // 원화를 외화로 환산
      String result = null;
